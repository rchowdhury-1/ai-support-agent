(function () {
  ...
  // Must capture currentScript synchronously — only valid during initial execution
  const script = document.currentScript || document.querySelector('script[data-agent-id]');
  const AGENT_ID = script ? script.dataset.agentId : null;
  const API_URL = (script ? script.dataset.apiUrl : null) || 'http://localhost:5001';
  ...
  function init() {
    ...
    // --- Build bubble button and inject into body ---
    ...
    console.log('[SupportAI] Widget initialised. Agent ID:', AGENT_ID, '| API:', API_URL);
  }
  ...
  // --- Self-initialise once DOM is ready ---
